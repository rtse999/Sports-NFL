...
# Behind LOS plays as proportion of total ---------------------------------
qb_pass_comparison <- 
  left_join(qb_los_epa, qb_epa, by = "passer_player_name") %>% 
  mutate(
    pass_to_los_pct = n_play.x / n_play.y
  ) %>% 
  select(passer_player_name, pass_to_los_pct)
